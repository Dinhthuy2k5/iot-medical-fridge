...
export default function Dashboard({ onLogout }) {
    // 1. Quản lý trạng thái đóng/mở của các cửa sổ Modal
    const [isRegisterOpen, setIsRegisterOpen] = useState(false);
    const [isProfileOpen, setIsProfileOpen] = useState(false);

    // 2. Quản lý dữ liệu tủ lạnh (Giữ nguyên logic IoT cũ)
    ...
    const fetchTemperatureLogs = async (deviceId) => {
        try {
            ...
            const formattedData = response.data.map(log => ({
                ...log, time: new Date(log.recordedAt).toLocaleTimeString('vi-VN'),
            })).reverse();
    ...
    // Các biến phụ trợ cho Widget và Biểu đồ
    ...
    const renderCustomDot = (props) => {
        ...
        return <circle cx={cx} cy={cy} r={6} stroke="white" strokeWidth={2} fill={isOut ? '#ef4444' : '#10b981'} />;
    };

    return (
        <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f1f5f9', minHeight: '100vh', position: 'relative' }}>

            {/* 1. THANH HEADER: Truyền quyền điều khiển các nút bấm xuống dưới */}
            <Header
                onLogout={onLogout}
            ...
            {/* 2. CÁC MODAL ẨN/HIỆN (Giấu code rối rắm đi) */}
            <ProfileModal isOpen={isProfileOpen} onClose={() => setIsProfileOpen(false)} />
            <RegisterModal isOpen={isRegisterOpen} onClose={() => setIsRegisterOpen(false)} />

            {/* 3. KHU VỰC CHỌN TỦ LẠNH VÀ WIDGETS GIÁM SÁT */}
            <div style={{ marginBottom: '20px', display: 'flex', gap: '20px', alignItems: 'center' }}>
                <div>
                    <label style={{ fontWeight: 'bold', marginRight: '10px', color: '#475569' }}>Chọn tủ giám sát:</label>
                    <select value={selectedDevice?.id || ''} onChange={(e) => setSelectedDevice(devices.find(d => d.id === e.target.value))} style={{ padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none', fontWeight: 'bold' }}>
                        {devices.map(device => (
            ...
            {selectedDevice && latestData && (
                <div style={{ display: 'flex', gap: '20px', marginBottom: '20px' }}>
                    <div style={{ flex: 1, backgroundColor: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
                        ...
                        <div style={{ fontSize: '36px', fontWeight: 'bold', color: isDanger ? '#ef4444' : '#10b981' }}>{latestData.temperature} °C</div>
                    ...
                    <div style={{ flex: 1, backgroundColor: isDanger ? '#fef2f2' : '#ecfdf5', padding: '20px', borderRadius: '12px', border: `1px solid ${isDanger ? '#fca5a5' : '#6ee7b7'}` }}>
                        ...
                        <div style={{ fontSize: '24px', fontWeight: 'bold', color: isDanger ? '#b91c1c' : '#047857' }}>{isDanger ? '⚠️ CẢNH BÁO NGUY HIỂM' : '✅ BÌNH THƯỜNG'}</div>
                        <small style={{ color: isDanger ? '#dc2626' : '#059669' }}>Ngưỡng an toàn: {selectedDevice.minTemp}°C - {selectedDevice.maxTemp}°C</small>
                    </div>
                </div>
            )}

            {/* 4. KHU VỰC CẢNH BÁO SỰ CỐ */}
            {alerts.length > 0 && (
                <div style={{ marginBottom: '20px' }}>
                    {alerts.map(alert => (
                        <div key={alert.id} style={{ backgroundColor: '#fee2e2', borderLeft: '6px solid #ef4444', padding: '15px 20px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                            ...
                            <button onClick={() => handleResolveAlert(alert.id)} style={{ backgroundColor: '#ef4444', color: 'white', border: 'none', padding: '12px 20px', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', fontWeight: 'bold' }}>
                                <CheckCircle size={20} style={{ marginRight: '8px' }} /> ĐÃ XỬ LÝ SỰ CỐ
                            </button>
                        </div>
                    ))}
                </div>
